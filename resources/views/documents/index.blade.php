...
<div class="doc-page-container">
    ...
    @php
        ...
        $docCount = $documents->filter(fn($d) => in_array(strtolower(pathinfo($d->latestVersion->file_path ?? '', PATHINFO_EXTENSION)), ['pdf', 'doc', 'docx']))->count();
        $designCount = $documents->filter(fn($d) => in_array(strtolower(pathinfo($d->latestVersion->file_path ?? '', PATHINFO_EXTENSION)), ['psd', 'ai', 'xd', 'fig', 'sketch']))->count();
        $imageCount = $documents->filter(fn($d) => in_array(strtolower(pathinfo($d->latestVersion->file_path ?? '', PATHINFO_EXTENSION)), ['png', 'jpg', 'jpeg', 'gif', 'svg', 'webp']))->count();
        $spreadsheetCount = $documents->filter(fn($d) => in_array(strtolower(pathinfo($d->latestVersion->file_path ?? '', PATHINFO_EXTENSION)), ['xls', 'xlsx', 'csv']))->count();
        $codeCount = $documents->filter(fn($d) => in_array(strtolower(pathinfo($d->latestVersion->file_path ?? '', PATHINFO_EXTENSION)), ['zip', 'sql', 'js', 'php', 'html', 'css', 'json']))->count();
    @endphp
    <div class="doc-stats-row">
        ...
        <div class="doc-stat-card stat-design">
            <span class="doc-stat-number">{{ $designCount }}</span>
            <span class="doc-stat-label">Design</span>
        </div>
    ...
    <div class="doc-filter-tabs">
        ...
        <button class="doc-filter-tab" data-filter="design">Design ({{ $designCount }})</button>
    ...
    <div class="doc-list-container">
        @forelse($documents as $doc)
            @php
                $ext = strtolower(pathinfo($doc->latestVersion->file_path ?? '', PATHINFO_EXTENSION));
                $iconClass = 'fa-file';
                $iconColor = '#64748b';
                $category = 'other';
                ...
                if (in_array($ext, ['pdf', 'doc', 'docx'])) {
                    ...
                    $iconColor = '#ef4444';
                    $category = 'dokumen';
                } elseif (in_array($ext, ['psd', 'ai', 'xd', 'fig', 'sketch'])) {
                ...
                } elseif (in_array($ext, ['zip', 'sql', 'js', 'php', 'html', 'css', 'json'])) {
            ...
            <div class="doc-list-item" data-category="{{ $category }}" data-name="{{ strtolower($doc->title) }}">
                <div class="doc-item-left">
                    ...
                    <div class="doc-item-info">
                        ...
                        @else
                            <a href="{{ route('documents.show', ['document' => $doc->id]) }}" class="doc-item-title">{{ $doc->title }}</a>
                        @endif
                        <div class="doc-item-meta">
                            ...
                            @if($isAttachment && isset($doc->source_task))
                                <span class="doc-meta-dot">•</span>
                                <span class="doc-source-badge">dari Tugas: {{ Str::limit($doc->source_task->title ?? '', 20) }}</span>
                            @endif
                        </div>
                    </div>
                </div>
                <div class="doc-item-right">
                    <div class="doc-user-info">
                        ...
                        <span class="doc-user-date"><i class="far fa-clock"></i> {{ $doc->updated_at->format('Y-m-d H:i') }}</span>
                    </div>
                    <div class="doc-actions-dropdown">
                        ...
                        <div class="doc-dropdown-menu">
                            ...
                            @if($isAttachment)
                                ...
                                <a href="{{ route('tasks.show', ['task' => $doc->source_task->id]) }}" class="doc-dropdown-item">
                                    <i class="fas fa-tasks"></i> Lihat Tugas
                                </a>
                                @endif
                                <button class="doc-dropdown-item doc-dropdown-danger" onclick="alert('Untuk menghapus, buka halaman detail tugas dan hapus dari sana.')">
                                    <i class="fas fa-trash"></i> Hapus
                                </button>
                            @elseif($doc->latestVersion && isset($doc->latestVersion->id))
                                <a href="{{ route('documents.download', ['version' => $doc->latestVersion->id]) }}" class="doc-dropdown-item">
                                    <i class="fas fa-download"></i> Download
                                </a>
                                ...
                                <form action="{{ route('documents.destroy', ['document' => $doc->id]) }}" method="POST" 
                                    onsubmit="return confirm('Yakin ingin menghapus dokumen ini beserta semua versinya?')">
                                    @csrf
                                    @method('DELETE')
...
<style>
    ...
    .stat-dokumen { border-left: 3px solid #ef4444; }
    .stat-design { border-left: 3px solid #8b5cf6; }
    .stat-gambar { border-left: 3px solid #ec4899; }
    .stat-spreadsheet { border-left: 3px solid #22c55e; }
    .stat-kode { border-left: 3px solid #3b82f6; }
