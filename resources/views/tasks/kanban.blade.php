        ...
        <script>
            ...
            function renderTasks() {
                ...
                columns.forEach(status => {
                    ...
                    // For done column, include both done and done_approved tasks
                    if (status === 'done') {
                        statusTasks = tasks.filter(t => t.status === 'done' || t.status === 'done_approved');
                    } else {
                        statusTasks = tasks.filter(t => t.status === status);
                    }

                    document.getElementById(`count-${status}`).textContent = statusTasks.length;

                    column.innerHTML = statusTasks.map(task => {
                        // Approval status indicator for done column
                        const approvalBadge = (task.status === 'done' || task.status === 'done_approved') ? `
                                            <span class="approval-badge ${task.status === 'done_approved' ? 'approved' : 'pending'}">
                                                <i class="fas fa-${task.status === 'done_approved' ? 'check-double' : 'clock'}"></i>
                                                ${task.status === 'done_approved' ? 'Approved' : 'Pending'}
                                            </span>
                                        ` : '';
